...
export function StatsGrowthContainer({ days }: Props) {
  ...
  const cumulativeConfig: ChartConfig = {
    cumulative: { label: t("stats.growth.cumulative_label"), color: "var(--chart-1)" },
  }
  const dailyConfig: ChartConfig = {
    new_users: { label: t("stats.growth.new_users_label"), color: "var(--chart-3)" },
  }
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
        <p className="text-sm font-semibold">{t("stats.growth.cumulative_title")}</p>
        <p className="mb-4 text-xs text-muted-foreground">{t("stats.growth.cumulative_description")}</p>
        <ChartContainer config={cumulativeConfig} className="h-60 w-full">
          <AreaChart data={data.items} margin={{ top: 4, right: 4, left: -8, bottom: 0 }}>
            <defs>
      ...
      <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
        ...
        <p className="mb-4 text-xs text-muted-foreground">{t("stats.growth.daily_description")}</p>
        <ChartContainer config={dailyConfig} className="h-60 w-full">
          <BarChart data={data.items} margin={{ top: 4, right: 4, left: -8, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" className="stroke-border/40" vertical={false} />
            <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(v) => v.slice(5)} />
