...
function StatKpiCard({ label, value }: { label: string; value: string | number }) {
  return (
...
export function StatsOverviewContainer({ days }: Props) {
  ...
  useEffect(() => {
    setLoading(true)
    statsApi.overview({ days }).then((res) => {
      setData(res)
      setLoading(false)
  ...
  const growthChartConfig: ChartConfig = {
    new_users: { label: t("stats.overview.new_users_label"), color: "var(--chart-3)" },
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-7">
        <StatKpiCard label={t("stats.overview.total_users")} value={data.total_users} />
        <StatKpiCard label={t("stats.overview.new_users_today")} value={data.new_users_today} />
        ...
        <StatKpiCard label={t("stats.overview.active_tenants")} value={data.active_tenants} />
        <StatKpiCard label={t("stats.overview.pending_applies")} value={data.pending_applies} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
          <p className="text-sm font-semibold">{t("stats.overview.active_trend_title")}</p>
          <p className="mb-4 text-xs text-muted-foreground">
            {t("stats.overview.active_trend_description")}
          </p>
          <ChartContainer config={activeChartConfig} className="h-52 w-full">
            <AreaChart data={data.active_users_trend} margin={{ top: 4, right: 4, left: -8, bottom: 0 }}>
              <defs>
        ...
        <div className="rounded-xl border border-border/70 bg-card p-4 shadow-sm">
          <p className="text-sm font-semibold">{t("stats.overview.growth_trend_title")}</p>
          <p className="mb-4 text-xs text-muted-foreground">
            {t("stats.overview.growth_trend_description")}
          </p>
          <ChartContainer config={growthChartConfig} className="h-52 w-full">
            <BarChart data={data.user_growth_trend} margin={{ top: 4, right: 4, left: -8, bottom: 0 }}>
              <CartesianGrid strokeDasharray="3 3" className="stroke-border/40" vertical={false} />
              <XAxis dataKey="date" tick={{ fontSize: 11 }} tickFormatter={(v) => v.slice(5)} />
              <YAxis tick={{ fontSize: 11 }} width={36} />
              <ChartTooltip content={<ChartTooltipContent />} />
              <Bar dataKey="new_users" fill="var(--chart-3)" radius={[3, 3, 0, 0]} />
            </BarChart>
          </ChartContainer>
        </div>
      </div>
